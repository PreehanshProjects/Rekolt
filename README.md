...
- `index.html`: current owner-supplied catalogue from 15 September 2026: 30 produce items and 14 pasta varieties. Existing produce prices and confirmed units are retained; products absent from the latest complete list are removed. Barquette Herbes retains its Rs 2,000 per barquette rate.
...
Pasta is grouped into Veg, Terre, Mer and Long Pasta. All 14 rates are per kg and included in order estimates. The five supplied dough flavours are listed separately; customers can request a flavour for a specific pasta in their order notes. No dough surcharge or compatibility is assumed. Old generic ravioli/lasagne and removed produce entries are dropped when saved orders load; they are not mapped to an arbitrary new pasta variety.
